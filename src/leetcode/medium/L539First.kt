...
fun main() {
    val output = findMinDifferenceFirst(listOf("01:00", "23:59"))
    print(output)
...
fun findMinDifferenceFirst(timePoints: List<String>): Int {
    if (timePoints.size == 1)
        return Math.min(minDiff, processTime(timePoints[0]))

    val mid = Math.floor((timePoints.size / 2).toDouble()).toInt()

    val first = findMinDifferenceFirst(timePoints.slice(0 until mid))
    val last = findMinDifferenceFirst(timePoints.slice(mid until timePoints.size))
...
fun processTime(time: String): Int {
    var diff = 0
    val hours = getHoursFirst(time)
    val minutes = getMinutesFirst(time)
...
fun getHoursFirst(time: String): String {
    return time.substringBefore(':')
}

fun getMinutesFirst(time: String): String {
